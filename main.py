...
parser.add_argument('--pred_path',           type=str,   default='./data/preds', help="Predictions directory.")
# HOG parameters
parser.add_argument('--hog',                 action='store_true', default=True,  help="Whether using HOG as a feature extraction method.")
parser.add_argument('--hog_orientations',    type=int,   default=9,        help="(HOG) Number of orientations.")
parser.add_argument('--hog_pix_per_cell',    type=int,   default=8,        help="(HOG) Number of pixels per cell.")
parser.add_argument('--hog_cells_per_block', type=int,   default=3,        help="(HOG) Number of cells per block.")
parser.add_argument('--hog_normalization',   type=str,   default='L2-Hys', help="(HOG) Normalization method.")
# kernel parameters
parser.add_argument('--ker',                 type=str,   default='Log',    help="Kernel chosen.")
parser.add_argument('--ker_gaussian_sigma',  type=float, default=1.,       help="(Gaussian kernel) sigma parameter.")
parser.add_argument('--ker_RBF_g',           type=float, default=1.,       help="(RBF kernel) g parameter.")
parser.add_argument('--ker_TStudent_p',      type=float, default=2.,       help="(T-Student kernel) p parameter.")
parser.add_argument('--ker_polynomial_a',    type=float, default=1.,       help="(Polynomial kernel) a parameter.")
parser.add_argument('--ker_polynomial_b',    type=float, default=0.,       help="(Polynomial kernel) b parameter.")
parser.add_argument('--ker_polynomial_p',    type=int,   default=2,        help="(Polynomial kernel) p parameter.")
parser.add_argument('--ker_chi2_gamma',      type=float, default=1.,       help="(Chi2 kernel) gamma parameter.")
parser.add_argument('--ker_wavelet_a',       type=float, default=1.,       help="(Wavelet kernel) a parameter.")
parser.add_argument('--ker_log_d',           type=float, default=2.,       help="(Log kernel) d parameter.")
parser.add_argument('--ker_GHI_beta',        type=float, default=1.,       help="(GHI kernel) beta parameter.")
# model parameters
parser.add_argument('--model',               type=str,   default='KRR',    help="Model chosen")
parser.add_argument('--model_KRR_lambda',    type=float, default=1e-4,     help="(KRR)  lambda parameter.")
parser.add_argument('--model_KSVC_C',        type=float, default=1,        help="(KSVC) C parameter.")
parser.add_argument('--model_KSVC_epsilon',  type=float, default=1e-3,     help="(KSVC) epsilon parameter.")
# other
parser.add_argument('--augment',             action='store_true', default=True, help="Whether using data augmentation.")
parser.add_argument('--augment_repeat',      type=int,   default=2,             help="(augmentation) number of times to repeat original dataset.")
